...
def main(times):
    ...
    # training parameters
    mb_size = 1
    x_dim = 3*2048 #49152 # RGB feature dimension
    y_dim = 110  # depth feature dimension
    hx_dim = 800 # RGB encoder E1 hidden layer dimension
    hy_dim = 800 # Depth encoder E2 hidden layer dimension

    space_dim_1 = 300 # RGB subspace representation dimension
    space_dim_2 = 300 # Depth subspace representation dimension
    ...
    def minibatch1(input, num_kernels=5, kernel_dim=3):
        ...
        print('ac: ', activation.shape)
        a = tf.expand_dims(activation, 3)
        b = tf.expand_dims(tf.transpose(activation, [1, 2, 0]), 0)
        print(a.shape, b.shape)
        diffs = a - b
        print('diff: ',diffs.shape)
        # import sys
        # sys.exit(0)
        abs_diffs = tf.reduce_sum(tf.abs(diffs), 2)
        print('abs: ',abs_diffs.shape)
        minibatch_features = tf.reduce_sum(tf.exp(-abs_diffs), 2)
        print('mini: ', minibatch_features.shape)
    ...
    def train_total():     
        ...
        for i in range(30001):
            ...
            # print(x_mb[0].shape, y_mb[0].shape, z_mb[0].shape)
            ...
            for upG in range(G_updates):
                x_mb, y_mb, z_mb = train_data.train_next_batch(mb_size)                
                noise_sample = sample_Noise(mb_size, noise_dim)
                g1_out, g2_out = sess.run([G1_solver, G2_solver], feed_dict={x:x_mb, y: y_mb,z: z_mb, noise:noise_sample})
                # print('Iteration {} g1_out {} and g2_out {}'.format(upG, g1_out, g2_out))
            # =====================
            #  Testing stage
            # =====================
            if i % 50 == 0:
                ...
                print('Iteration = ',i, '  Accuracy:  f_subj = %.4f'% acc_te_r1f2_num, '  s_subj = %.4f'% acc_te_f1r2_num, '  f_s = %.4f'% acc_te_r1r2_num)
