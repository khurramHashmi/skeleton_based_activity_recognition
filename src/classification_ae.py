...
def train(step_count_tb, reshape_size, draw_graph=False, graph_out=''):
    ...
    for data, targets, __, frame_count in train_loader:
        ...
        if draw_graph:  # whether to draw graph for frame distribution or not
            ...
            barplot('activity', 'frame_count', pd.DataFrame({'activity': activity, 'frame_count': frame_count.numpy()}),
                    'batch_{}'.format(batch_count), graph_out)
...
parser.add_argument("-ac", "--ae_checkpoint", help="path to load autoencoder from",
                    default="autoencoder_weights/subject_skeleton_autoencoder_int_rgb.pth")
...
if args.train:
    ...
    # model = TransformerModel(ntokens, emsize, nhead, nhid, nlayers, args.batch_size, args.frame_count, dropout).to(
    #     device)
    model = classification_network().to(device)
    ...
    for epoch in range(0, max_epochs):
        ...
        if epoch == max_epochs - 1:
            ...
            val_loss, val_acc = evaluate(model, eval_loader, 100, draw_img=True, visualize=False,
