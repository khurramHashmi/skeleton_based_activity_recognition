...
def normalize(path='', out_path='./', pickle_output='./', part='train'):
    ...
    for j in tqdm.tqdm(range(len(data))):
        ...
        # skeleton_data = np.load(data.iloc[j,0], allow_pickle=True).item()
        skeleton_data = pd.read_csv(data.iloc[j, 0], delimiter="\t", header=None)


        # skeleton_sequence = skeleton_data['values']#
        skeleton_sequence = np.array(skeleton_data[1].tolist())[0]
        ...
        write_path = os.path.join(out_path, '.'.join(os.path.basename(data.iloc[j,0]).split('.')[:2])+'.csv') #os.path.join(out_path, os.path.basename(data.iloc[j,0].split('.')[0]))
...
parser.add_argument("-d", "--data_path",  default='./xsub_val_skel_float.csv', help="Path to dataset to run script on")
parser.add_argument("-o", "--out_path",  default='./data/data_skel_float/', help="Path to store files")
